...
def checkfile(hourname,hourbiddernumber):
  file_exist = 0
  allfiles = glob.glob('../../FeatureExtraction/Results/HourBidder/%s/%s_*' % (hourname,hourname))
  if eval(hourbiddernumber) == len(allfiles):
    file_exist = 1
  if file_exist > 0:
    return 1
  else:
...
def singleJob(node, job):
  ...
  for filename in nodefilelist:
    hourname = filename[0]
    hourbiddernumber = filename[1]
    try:
      if checkfile(hourname,hourbiddernumber):
        logging.info('\t--< (priority) %d (node)%s (filename) %s' %(priority, node, hourname))
        fail_penalty = 0
      else:
        logging.info('\t--> (priority) %d (node)%s (filename) %s' %(priority, node, hourname))
        os.system(""" ssh -o StrictHostKeyChecking=no %s 'cd /cs/taatto/group/urenzyme/workspace/FacebookChallenge/FeatureExtraction/Bin/; python generate_hour_bidder_file_single_time.py %s' """ % (node,hourname) )
        #print(""" ssh -o StrictHostKeyChecking=no %s 'cd /cs/taatto/group/urenzyme/workspace/FacebookChallenge/FeatureExtraction/Bin/; python generate_time_bidder_file_single_time.py %s' """ % (node,hourname) )
        logging.info('\t--| (priority) %d (node)%s (filename) %s' %(priority, node, hourname))
        fail_penalty = -1
        time.sleep(1)
      ...
      logging.info('\t--= (priority) %d (node)%s (filename) %s' %(priority, node, hourname))
      fail_penalty = 1
    if not checkfile(hourname,hourbiddernumber):
      job_queue.put((priority,job_detail))
      logging.info('\t--x (priority) %d (node)%s (filename) %s' %(priority, node, hourname))
      fail_penalty = 1
  time.sleep(5)
  return fail_penalty
  pass # def
...
def run():
  ...
  # get list of files
  hourlist = []
  hourbiddernumberlist = []
  for line in open('../../FeatureExtraction/Results/hourinformation'):
    words = line.strip().split(',')
    hourlist.append(words[0])
    hourbiddernumberlist.append(words[3])
  # generate job queueo
  jobind = 0
  n=1
  nodefilelist = []
  for hourindex in range(len(hourlist)):
    hourname = hourlist[hourindex]
    hourbiddernumber = hourbiddernumberlist[hourindex]
    if checkfile(hourname,hourbiddernumber):
        continue
    if n>1:
      ...
      n=1
      nodefilelist = []
    nodefilelist.append((hourname,hourbiddernumber))
    n+=1
