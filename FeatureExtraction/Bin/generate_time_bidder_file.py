  ...
  for filename in nodefilelist:
    ...
    try:
      if checkfile(hourname,hourbiddernumber):
        ...
        logging.info('\t--> (priority) %d (node)%s (filename) %s' %(priority, node, hourname))
        os.system(""" ssh -o StrictHostKeyChecking=no %s 'cd /cs/taatto/group/urenzyme/workspace/FacebookChallenge/FeatureExtraction/Bin/; python generate_time_bidder_file_single_time.py %s' """ % (node,hourname) )
