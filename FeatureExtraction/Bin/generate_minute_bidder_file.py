...
def checkfile(minutename,minutebiddernumber):
  file_exist = 0
  allfiles = glob.glob('../../FeatureExtraction/Results/MinuteBidder/%s/%s_*' % (minutename,minutename))
  if eval(minutebiddernumber) == len(allfiles):
    file_exist = 1
  if file_exist > 0:
    return 1
  else:
...
def singleJob(node, job):
  ...
  for filename in nodefilelist:
    minutename = filename[0]
    minutebiddernumber = filename[1]
    try:
      if checkfile(minutename,minutebiddernumber):
        logging.info('\t--< (priority) %d (node)%s (filename) %s' %(priority, node, minutename))
        fail_penalty = 0
      else:
        logging.info('\t--> (priority) %d (node)%s (filename) %s' %(priority, node, minutename))
        os.system(""" ssh -o StrictHostKeyChecking=no %s 'cd /cs/taatto/group/urenzyme/workspace/FacebookChallenge/FeatureExtraction/Bin/; python generate_minute_bidder_file_single_time.py %s' """ % (node,minutename) )
        #print(""" ssh -o StrictHostKeyChecking=no %s 'cd /cs/taatto/group/urenzyme/workspace/FacebookChallenge/FeatureExtraction/Bin/; python generate_time_bidder_file_single_time.py %s' """ % (node,minutename) )
        logging.info('\t--| (priority) %d (node)%s (filename) %s' %(priority, node, minutename))
        fail_penalty = -1
        time.sleep(1)
      ...
      logging.info('\t--= (priority) %d (node)%s (filename) %s' %(priority, node, minutename))
      fail_penalty = 1
    if not checkfile(minutename,minutebiddernumber):
      job_queue.put((priority,job_detail))
      logging.info('\t--x (priority) %d (node)%s (filename) %s' %(priority, node, minutename))
      fail_penalty = 1
  time.sleep(5)
  return fail_penalty
  pass # def
...
def run():
  ...
  # get list of files
  minutelist = []
  minutebiddernumberlist = []
  for line in open('../../FeatureExtraction/Results/minuteinformation'):
    words = line.strip().split(',')
    minutelist.append(words[0])
    minutebiddernumberlist.append(words[3])
  # generate job queueo
  jobind = 0
  n=1
  nodefilelist = []
  for minuteindex in range(len(minutelist)):
    minutename = minutelist[minuteindex]
    minutebiddernumber = minutebiddernumberlist[minuteindex]
    if checkfile(minutename,minutebiddernumber):
        continue
    if n>1:
      ...
      n=1
      nodefilelist = []
    nodefilelist.append((minutename,minutebiddernumber))
    n+=1
