...
def checkfile(filename):
  file_exist = 0
  file_exist += os.path.isfile("../../FeatureExtraction/Results/BidderFeatures/%s" % filename)
  if file_exist > 0:
    return 1
  else:
    return 0
  ...
  for filename in nodefilelist:
    try:
      if checkfile(filename):
        ...
        logging.info('\t--> (priority) %d (node)%s (filename) %s' %(priority, node, filename))
        os.system(""" ssh -o StrictHostKeyChecking=no %s 'cd /cs/taatto/group/urenzyme/workspace/FacebookChallenge/FeatureExtraction/Bin/; python feature_extraction_single_file.py %s' """ % (node,filename) )
        #print(""" ssh -o StrictHostKeyChecking=no %s 'cd /cs/taatto/group/urenzyme/workspace/FacebookChallenge/FeatureExtraction/Bin/; python feature_extraction_single_file.py %s' """ % (node,filename) )
        logging.info('\t--| (priority) %d (node)%s (filename) %s' %(priority, node, filename))
        fail_penalty = -1
        time.sleep(1)
