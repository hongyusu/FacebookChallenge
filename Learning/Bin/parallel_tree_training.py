...
def singleJob(node, job):
  ...
  (paramind,K,k,c,outfilename) = job_detail
  ...
  try:
    if checkfile(filename):
      ...
      os.system(""" ssh -o StrictHostKeyChecking=no %s 'cd /cs/taatto/group/urenzyme/workspace/FacebookChallenge/Learning/Bin/; nohup matlab -nodisplay -nosplash -r "with_treebagger_train '%s' '%s' '%s' '%s' '%s'" > /var/tmp/tmp'  """ % (node,paramind,K,k,c,outfilename) )
      logging.info('\t--| (priority) %d (node)%s (filename) %s' %(priority, node, filename))
      fail_penalty = -1
      time.sleep(1)
...
def run():
  ...
  paramind = 0
  K = 10 
  for cstr in range(5,200,5):
    c = '%s' % cstr
    for k in range(1,(K+1)):
      paramind += 1
      outfilename = '../../Learning/Results/ParameterSelection/tree_%d' % paramind
      if checkfile(outfilename):
        continue
      job_queue.put((paramind,(str(paramind),str(K),str(k),c,outfilename)))
  # get computing node
